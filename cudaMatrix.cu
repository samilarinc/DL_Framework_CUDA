...
__global__ void cof(double *a, double *b, int i, int j, int N){
    ...
    cuda1DArray::cuda1DArray(int height, int width, double* arr){
        cudaError_t err = cudaSuccess;
        this->height = height;
        this->width = width;
        size = height*width*sizeof(double);
    ...
    void cuda1DArray::gaussianElimination(double* dev_a, int N) {
        int i;
        for (i = 0; i<N; i++) {
            scale << <1, 1 >> >(dev_a, N, i);
    ...
    double cuda1DArray::det(){
        ...
        cudaMemcpy(dev_a, data, N*N * sizeof(double), cudaMemcpyHostToDevice);
    ...
    cuda1DArray cuda1DArray::inv(){
        ...
        return adjoint().divConstant(determinant);
    }
